...
| --- | --- | --- |
| `@plumbus/core` | 0.7.1 | 0.8.0-beta.2 (beta.1 + host data-plane resolver exports, AI provider admission hooks, `ctx.flows.terminate`) |
...
No database schema changes are introduced by this family beyond the tenant data-plane helpers, which are opt-in — a host enabling spine dispatch needs the framework's `opaque_dispatch` table on its control plane (`migrations/spine/0000_opaque_dispatch.sql`) and the `core_plumbus` durable tables on each tenant plane (the generator already emits them).
