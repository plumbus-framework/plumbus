...
**`chatConfirmAction` (Path A) is decision-only by default; opt in to execution with `policy.action.frameworkExecuteOnConfirm: true`.** With the flag unset (default `false`), a confirmed Path-A action validates, marks the pending row confirmed, emits domain events, and returns without side effects (historical behavior). With the flag `true`, the confirmed action executes through the framework capability pipeline (`executeCapability`, access policy enforced). This is **not** a blanket breaking change — the default is unchanged. Provider-native tool calling (Path B, below) always executes on confirm and resumes the turn for a single answer-only completion (no further tool rounds or nested confirmation).
