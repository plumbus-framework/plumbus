...
2. **`ChatTurn`** gains `logical_turn_id TEXT`, `continuation_of_turn_id TEXT`, `tools_executed JSONB`, then the **unique** index `chat_turn_session_id_ordinal_idx`. Generated DDL adds the columns first. The unique index assumes `(session_id, ordinal)` is already unique (assigned by `appendTurn` from `existing.length`), which holds for most tables. **Caveat:** the old ordinal allocation is racy under concurrency (read-max-plus-insert; see [design/session-entities.md](./design/session-entities.md)), so a pre-existing high-concurrency table may already hold duplicate `(session_id, ordinal)` rows — the index build then **fails**. Run a dedupe / verification query (e.g. `SELECT session_id, ordinal, count(*) FROM chat_turn GROUP BY 1, 2 HAVING count(*) > 1`) to confirm there are no duplicates before migrating.

3. **`ChatPendingAction`** is a transient (≤15 min TTL) table: `schema_hash` is **retained** (nullable, superseded — not renamed or dropped), and `input_schema_hash` + `tool_binding_hash` are **added** as `NOT NULL DEFAULT ''`. This is an additive `ADD COLUMN` migration — no rename, no drop, no backfill. `status` gains `confirming` / `failed` / `indeterminate`, and `version` / `expected_session_revision` / `attempt_id` / `claimed_at` / `execution_started_at` / `completed_at` / `resume_payload` / `confirmation_projection` are added. Regenerate with `plumbus migrate` after `plumbus generate`; no data migration is required for rows that will expire.

## Storage requirements

`ChatConversationStore` requires `Repository.updateWhere` on `ChatSession` and `ChatPendingAction`. Adapters without conditional writes fail closed on the first turn/confirmation with `chat.storage_unsupported` (the check runs in the turn/confirm handlers, not at server boot).
