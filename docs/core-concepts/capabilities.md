...
- **Full pipeline:** nested calls run through `executeCapability` (static access, input validation, input-aware authorization, approval gate, audit, output validation). The callee inherits the caller's auth, transaction scope, and correlation context.
