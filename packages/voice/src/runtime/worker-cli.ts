...
async function main(): Promise<void> {
  if (process.argv.includes('--help')) {
    console.log(
      'Usage: node --import tsx packages/voice/src/runtime/worker-cli.ts [--room <roomName>]',
    );
  ...
  if (!roomName) {
    throw new PlumbusError(
      ...
      'Specify --room <roomName> or set VOICE_AGENT_ROOM',
    );
  }
  ...
  if (!voice) {
    throw new PlumbusError(
      ErrorCode.NotFound,
      'No livekit voice definitions found under app/voices/',
    );
  }
  ...
  const handle = await joinVoiceRoomSession({
    ...
    createExecutionContext: () => {
      throw new PlumbusError(
        ...
        'worker-cli requires app-specific createExecutionContext wiring via plumbus voice worker',
      );
    },
  });
