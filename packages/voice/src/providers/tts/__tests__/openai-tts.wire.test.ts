...
describe('OpenAI TTS wire protocol', () => {
  it('posts speech synthesis requests with auth and streaming accept header', async () => {
    const fetcher = vi.fn(async (url: string, init?: { method?: string; headers?: Record<string, string>; body?: string }) => {
    ...
    for await (const chunk of provider.synthesizeStream!('Hello there', provider.mapDeliveryTone({}))) {
      chunks.push(chunk);
    }
