...
| **`@plumbus/knowledge-base`** | **You are here.** Scoped knowledge providers + registry. |
| [`@plumbus/chat`](../chat/) | Server-side conversational runtime — consumes registered sources via `knowledgeContext`. |
| [`@plumbus/chat-ui`](../chat-ui/) | React UI for `@plumbus/chat`. |
| [`@plumbus/mcp`](../mcp/) | Serve capabilities to AI agents over MCP. |
