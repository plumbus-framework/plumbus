...
export function generateApiDocs(
  ...
  for (const cap of caps) {
    ...
    const lines = [
      ...
      '| `csrf_failed` | Session CSRF / Origin check failed (mutating methods) |',
      '| `forbidden` | Access denied |',
      '| `missing_scope` | Authenticated but missing required scope |',
      '| `authentication_unavailable` | Auth backend temporarily unavailable |',
