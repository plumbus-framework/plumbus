...
| [action-confirmation.md](./action-confirmation.md) | The v0.1 confirm() gap — apps that ship action-confirmation flows must call `chatConfirmAction` directly. |
...
- **`useChat.confirm(actionId)` is a UI-only stub in v0.1.** It clears local `pendingConfirmation` state and does NOT call the server. Apps that ship action confirmation MUST call `chatConfirmAction` directly with `{ actionId, capabilityName, schemaHash, execute }` — see action-confirmation.md.
