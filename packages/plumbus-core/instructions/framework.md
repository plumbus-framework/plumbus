...
For core **0.7.0** upgrades, read [the security release checklist](./upgrading-security-release.md), then run `plumbus init --patch` to refresh agent wiring to **v16**.
...
| **Translation** | i18n catalog of localized strings consumed via `ctx.translations` | `defineTranslation()` |
...
| `ctx.translations` | Localized string resolution — `ctx.translations.t("errors.notFound")` |
| `ctx.capabilities` | Cross-capability invocation — `ctx.capabilities.invoke('domain.name', input)` |

## Consumption Surfaces

Capabilities can be consumed by:

| Surface | Default | Opt-in |
|---------|---------|--------|
| **HTTP** (REST) | Auto-generated for every `query` / `action` / `job` capability | — |
| **Partner API** | Off | `exposeAs: ['api']` + `@plumbus/api` + `registerApiRoutes()` in `app/server.ts` |
| **Typed TS clients** (`@plumbus/ui`) | Generated by `plumbus ui generate` | — |
| **MCP** (Model Context Protocol) | Off | Add `exposeAs: ['mcp']` to selected capabilities; install `@plumbus/mcp`; run `plumbus mcp serve` |

The same `executeCapability()` pipeline (validation → access → handler → audit) runs for every surface. See `node_modules/@plumbus/core/instructions/mcp.md` (core surface) and `node_modules/@plumbus/mcp/instructions/README.md` (runtime recipes) for MCP.

## How Subsystems Connect

1. **Capabilities** are the only entry points for business logic. HTTP routes are auto-generated from capability contracts; selected capabilities may additionally be exposed to AI agents via MCP (`exposeAs: ['mcp']`).
...
app/
  ...
  translations/
    <name>.translation.ts  # Translation catalog
config/
