...
The same `executeCapability()` pipeline (validation → access → handler → audit) runs for every surface. See `node_modules/@plumbus/core/instructions/mcp.md` for the MCP guide.
