...
**Per-request accounting** uses `estimateModelCost()` and the fixed pricing catalog. The legacy `calculateModelCost()` and generation `cost` remain numeric for compatibility, but an unknown model produces `costAvailable: false`; it must be recorded as `null` in a custom ledger. Use `aggregatedCostAvailable` for tool-loop totals and `costAvailable` for embedding callbacks. Explicit free costs remain zero/available. Cached-token and long-context adjustments use provider usage metadata. Do not add automatic pricing refresh or promotion lifecycle logic.
