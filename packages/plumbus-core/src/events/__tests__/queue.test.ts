...
describe("InMemoryQueue", () => {
  it("delivers published events to subscribers", async () => {
    ...
    queue.subscribe(async (e) => { received.push(e); });
  ...
  it("delivers to multiple subscribers", async () => {
    ...
    const a: string[] = [];
    const b: string[] = [];
    queue.subscribe(async (e) => { a.push(e.id); });
    queue.subscribe(async (e) => { b.push(e.id); });
  ...
  it("unsubscribe removes handler", async () => {
    ...
    const unsub = queue.subscribe(async (e) => { received.push(e.id); });
    unsub();
  ...
  it("swallows subscriber errors without affecting others", async () => {
    const queue = createInMemoryQueue();
    const received: string[] = [];
    queue.subscribe(async () => { throw new Error("boom"); });
    queue.subscribe(async (e) => { received.push(e.id); });
