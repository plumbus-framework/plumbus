...
function makeConsumer(id: string, eventTypes: string[], versionConstraint?: string): EventConsumer {
  return {
    id,
    eventTypes,
    versionConstraint,
...
describe('ConsumerRegistry', () => {
  ...
  it('throws on duplicate consumer ID', () => {
    ...
    expect(() => reg.register(makeConsumer('c1', ['order.updated']))).toThrow('already registered');
  });
  ...
  it('getAll returns all consumers', () => {
    const reg = new ConsumerRegistry();
    reg.registerAll([makeConsumer('a', ['x']), makeConsumer('b', ['y'])]);
