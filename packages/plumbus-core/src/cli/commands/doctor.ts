...
export function checkPlumbusCore(): DoctorCheck {
  try {
    ...
    return { name: "plumbus-core", status: "warn", message: "plumbus-core not found in node_modules (may be workspace root)" };
  } catch {
...
export function checkAppStructure(): DoctorCheck {
  ...
  if (existing.length > 0) {
    return { name: "app-structure", status: "warn", message: `Missing: ${dirs.filter((d) => !existing.includes(d)).join(", ")}` };
  }
  return { name: "app-structure", status: "fail", message: "app/ directory not found — run `plumbus create`" };
}
...
export async function checkPostgreSQL(): Promise<DoctorCheck> {
  try {
    const configPath = resolvePath("config", "app.config.ts");
    if (!fs.existsSync(configPath)) {
      return { name: "postgresql", status: "warn", message: "Config not found — cannot test PostgreSQL" };
    }
    ...
    return await new Promise<DoctorCheck>((resolve) => {
      ...
      const timeout = setTimeout(() => {
        socket.destroy();
        resolve({ name: "postgresql", status: "fail", message: `PostgreSQL not reachable at ${config.database.host}:${config.database.port}` });
      }, 3000);
      socket.connect(config.database.port, config.database.host, () => {
        clearTimeout(timeout);
        socket.destroy();
        resolve({ name: "postgresql", status: "ok", message: `PostgreSQL reachable at ${config.database.host}:${config.database.port}` });
      });
      socket.on("error", () => {
        clearTimeout(timeout);
        socket.destroy();
    ...
    return { name: "postgresql", status: "warn", message: "Could not check PostgreSQL connectivity" };
  }
}
...
export async function checkRedis(): Promise<DoctorCheck> {
  try {
    ...
    return await new Promise<DoctorCheck>((resolve) => {
      ...
      const timeout = setTimeout(() => {
        socket.destroy();
        resolve({ name: "redis", status: "fail", message: `Redis not reachable at ${config.queue.host}:${config.queue.port}` });
      }, 3000);
      socket.connect(config.queue.port, config.queue.host, () => {
        clearTimeout(timeout);
        socket.destroy();
        resolve({ name: "redis", status: "ok", message: `Redis reachable at ${config.queue.host}:${config.queue.port}` });
      });
      socket.on("error", () => {
        clearTimeout(timeout);
        socket.destroy();
