...
import { buildGenerateOpenApiDocument } from './generate-openapi-from-zod.js';
import {
...
export {
  ...
  toJsonSchema2020,
  toOpenApi31Document,
} from './generate-openapi-from-zod.js';
export type { OpenApi31ConvertibleDocument } from './generate-openapi-from-zod.js';
...
export function generateCapabilityNameType(capabilities: CapabilityContract[]): string {
  ...
  const names = capabilities.map((c) => `"${getCanonicalCapabilityName(c)}"`).join(' | ');
...
export function generatePlumbusDeclaration(
  ...
  // capabilityName
  if (capabilities.length > 0) {
    const capNames = capabilities.map((c) => `"${getCanonicalCapabilityName(c)}"`).join(' | ');
...
/**
 * Typed fetch helper for a capability.
 * `plumbus generate` does not write this to disk — use `plumbus ui generate`.
 */
...
/**
 * React hook helper for a capability.
 * `plumbus generate` does not write this to disk — use `plumbus ui generate`.
 */
...
/** Generate a mermaid capability dependency graph from declared effects edges. */
export function generateCapabilityGraphMarkdown(capabilities: CapabilityContract[]): string {
  const lines = [
    '# Capability dependency graph',
    '',
    'Auto-generated by `plumbus generate` from declared capability effects.',
    '',
    'Solid arrows: `effects.capabilities` invoke dependencies. Dotted arrows: data, events, flows, and external effects.',
    '',
    '```mermaid',
    'flowchart LR',
  ];
  ...
  for (const cap of capabilities) {
    const from = getCanonicalCapabilityName(cap);
    ensureNode(sanitizeMermaidId(from), from);

    for (const dep of cap.effects.capabilities ?? []) {
      ensureNode(sanitizeMermaidId(dep), dep);
      lines.push(`  ${sanitizeMermaidId(from)} --> ${sanitizeMermaidId(dep)}`);
    }

    for (const entity of cap.effects.data ?? []) {
      const targetId = sanitizeMermaidId(`data_${entity}`);
      ensureNode(targetId, `data:${entity}`);
      lines.push(`  ${sanitizeMermaidId(from)} -.->|data| ${targetId}`);
    }

    for (const eventName of cap.effects.events ?? []) {
      const targetId = sanitizeMermaidId(`event_${eventName}`);
      ensureNode(targetId, `event:${eventName}`);
      lines.push(`  ${sanitizeMermaidId(from)} -.->|event| ${targetId}`);
    }

    for (const flowName of cap.effects.flows ?? []) {
      const targetId = sanitizeMermaidId(`flow_${flowName}`);
      ensureNode(targetId, `flow:${flowName}`);
      lines.push(`  ${sanitizeMermaidId(from)} -.->|flow| ${targetId}`);
    }

    for (const external of cap.effects.external ?? []) {
      const targetId = sanitizeMermaidId(`external_${external}`);
      ensureNode(targetId, `external:${external}`);
      lines.push(`  ${sanitizeMermaidId(from)} -.->|external| ${targetId}`);
    }

    if (cap.effects.ai) {
      const targetId = sanitizeMermaidId(`ai_${from}`);
      ensureNode(targetId, 'ai:true');
      lines.push(`  ${sanitizeMermaidId(from)} -.->|ai| ${targetId}`);
    }
  }
...
/**
 * Remove leftover fetch/hook files that older `plumbus generate` wrote under
 * `{outputDir}/clients/`. Typed app clients come from `plumbus ui generate`.
 */
function wipeLeftoverGeneratedClients(outputDir: string): void {
  const dir = path.join(outputDir, 'clients');
...
export function generateAll(
  ...
  wipeLeftoverGeneratedClients(outputDir);
  ...
  // OpenAPI spec (3.1.0, JSON Schema 2020-12 nullables — same conversion as @plumbus/api)
  const openApi = buildGenerateOpenApiDocument(capabilities);
  ...
  writeFile(
    path.join(outputDir, 'capability-graph.md'),
    generateCapabilityGraphMarkdown(capabilities),
  );
  generated.push('capability-graph.md');
...
export function registerGenerateCommand(program: Command): void {
  program
    .command('generate')
    .description('Generate derived artifacts from contracts (types, OpenAPI, manifest)')
