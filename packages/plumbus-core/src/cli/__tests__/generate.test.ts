...
describe('Entity type generation', () => {
  describe('generateEntityInterface', () => {
    ...
    it('marks required fields as non-optional', () => {
      const result = generateEntityInterface(mockEntity());
      const recordSection = result.split('export interface UserRecord {')[1]?.split('}')[0] ?? '';
      expect(recordSection).toContain('  email: string;');
      expect(recordSection).not.toContain('email?');
    });
    ...
    it('generates CreateInput that omits system fields', () => {
      const result = generateEntityInterface(mockEntity());
      expect(result).toContain('export interface UserCreateInput {');
      expect(result).toContain('export interface UserUpdateInput {');
      // CreateInput should contain user-defined fields
      const createSection =
        result.split('export interface UserCreateInput {')[1]?.split('}')[0] ?? '';
      expect(createSection).toContain('email: string;');
      expect(createSection).toContain('name?:');
      // CreateInput should NOT contain system fields
      expect(createSection).not.toContain('id:');
      expect(createSection).not.toContain('createdAt');
      expect(createSection).not.toContain('updatedAt');
    });

    it('generates UpdateInput with all non-system fields optional', () => {
      const result = generateEntityInterface(mockEntity());
      const updateSection =
        result.split('export interface UserUpdateInput {')[1]?.split('}')[0] ?? '';
      expect(updateSection).toContain('email?: string;');
      ...
      expect(updateSection).not.toContain('updatedAt');
    });

    it('omits tenantId from CreateInput for tenant-scoped entities', () => {
      const entity = mockEntity({ tenantScoped: true });
      const result = generateEntityInterface(entity);
      const createSection =
        result.split('export interface UserCreateInput {')[1]?.split('}')[0] ?? '';
      expect(createSection).not.toContain('tenantId');
    });
  });
  ...
  describe('generateDataServiceMap', () => {
    it('generates a DataServiceMap with repository types', () => {
      ...
      expect(result).toContain(
        'Order: Repository<OrderRecord, OrderCreateInput, OrderUpdateInput>;',
      );
    });
  });
...
describe('generatePlumbusDeclaration', () => {
  ...
  it('generates never for empty collections', () => {
    ...
    expect(result).toContain('eventPayloads: Record<string, never>');
  ...
  it('generates event payload map with typed payloads', () => {
    const events = [
      mockEvent({ name: 'user.created', payload: z.object({ userId: z.string() }) }),
      mockEvent({
        name: 'order.completed',
        payload: z.object({ orderId: z.string(), total: z.number() }),
      }),
    ];
    const result = generatePlumbusDeclaration([], events, [], []);

    expect(result).toContain('eventPayloads: {');
    expect(result).toContain('"user.created":');
    expect(result).toContain('userId: string;');
    expect(result).toContain('"order.completed":');
    expect(result).toContain('orderId: string;');
    expect(result).toContain('total: number;');
  });
  ...
  it('handles multiple entities in the entities map', () => {
    ...
    expect(result).toContain('User: Repository<UserRecord, UserCreateInput, UserUpdateInput>');
    expect(result).toContain('Order: Repository<OrderRecord, OrderCreateInput, OrderUpdateInput>');
    expect(result).toContain(
      'import type { UserRecord, UserCreateInput, UserUpdateInput } from "./entity-types.js"',
    );
    expect(result).toContain(
      'import type { OrderRecord, OrderCreateInput, OrderUpdateInput } from "./entity-types.js"',
    );
  });
});
