...
describe("plumbus certify policy", () => {
  ...
  it("provides remediation recommendations", () => {
    const report = evaluatePolicy(
      ...
      [mockEntity({
        tenantScoped: false,
        fields: {
          secret: { type: "string", options: { classification: FieldClassification.HighlySensitive } },
        },
      })],
    );
  ...
  it("evaluates HIPAA policy with masking rule", () => {
    const report = evaluatePolicy(
      ...
      [mockEntity({
        fields: {
          diagnosis: { type: "string", options: { classification: FieldClassification.Sensitive, encrypted: true } },
        },
      })],
    );
