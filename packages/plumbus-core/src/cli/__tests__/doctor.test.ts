...
describe('plumbus doctor', () => {
  ...
  it('checkAgentWiring treats generated CLAUDE.md as agent wiring', () => {
    const tempDir = mkdtempSync(path.join(tmpdir(), 'plumbus-doctor-agent-wiring-'));
    try {
      vi.spyOn(process, 'cwd').mockReturnValue(tempDir);
      writeFileSync(path.join(tempDir, 'CLAUDE.md'), generateClaudeMd(false), 'utf-8');
  ...
  it('checkAgentWiring ignores non-generated CLAUDE.md files', () => {
    const tempDir = mkdtempSync(path.join(tmpdir(), 'plumbus-doctor-agent-wiring-'));
    try {
      vi.spyOn(process, 'cwd').mockReturnValue(tempDir);
      writeFileSync(
