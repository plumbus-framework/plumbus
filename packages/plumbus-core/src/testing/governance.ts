 ...
 */
export function assertGovernanceSignals(
 ...
 */
export function assertMaxSeverity(
  result: GovernanceResult,
  maxSeverity: GovernanceSeverity,
): void {
  const severityOrder = { info: 0, warning: 1, high: 2 };
  const maxLevel = severityOrder[maxSeverity] ?? 0;
  const violations = result.effective.filter(
    (s) => (severityOrder[s.severity] ?? 0) > maxLevel,
  );
...
export function assertPolicyNonCompliance(
  ...
  if (report.score >= 100) {
    throw new Error(
      `Expected policy "${profileName}" to FAIL but it passed with ${report.score}%`,
    );
  }
}
