...
function makeResult(
  signals: Array<{ rule: string; severity: string; description: string }>,
  ...
  const effective = mappedSignals.filter((s) => !overrideApplied.includes(s.rule));
  return {
...
describe('assertGovernanceSignals', () => {
  ...
  it('throws when expected rule did not fire', () => {
    const result = makeResult([{ rule: 'rule-1', severity: 'warning', description: 'test' }]);
...
describe('assertMaxSeverity', () => {
  ...
  it('passes for info max when only info signals exist', () => {
    const result = makeResult([{ rule: 'r1', severity: 'info', description: 'ok' }]);
...
describe('assertPolicyCompliance', () => {
  ...
  it('throws for non-compliant inventory', () => {
    ...
    expect(() => assertPolicyCompliance(inventory, 'internal_security_baseline')).toThrow(
      'compliance',
    );
  });
});
...
describe('assertPolicyNonCompliance', () => {
  ...
  it('throws when inventory passes policy', () => {
    const inventory = emptyInventory();
    expect(() => assertPolicyNonCompliance(inventory, 'internal_security_baseline')).toThrow(
      'FAIL',
    );
  });
});
