...
function echoCapability(): CapabilityContract<z.ZodObject<{ message: z.ZodString }>, z.ZodObject<{ echo: z.ZodString }>> {
  return {
    name: "echo",
...
describe("runCapability", () => {
  ...
  it("returns forbidden error when user lacks required role", async () => {
    const result = await runCapability(adminOnlyCapability(), { value: 1 }, {
