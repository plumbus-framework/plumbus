...
export function createErrorService(): ErrorService {
  return {
    validation: (message, metadata) =>
      createError(ErrorCode.Validation, message, metadata),
    notFound: (message, metadata) =>
      createError(ErrorCode.NotFound, message, metadata),
    forbidden: (message, metadata) =>
      createError(ErrorCode.Forbidden, message, metadata),
    conflict: (message, metadata) =>
      createError(ErrorCode.Conflict, message, metadata),
    internal: (message, metadata) =>
      createError(ErrorCode.Internal, message, metadata),
  };
}
