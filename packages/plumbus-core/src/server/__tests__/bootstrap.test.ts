...
vi.mock("fastify", () => {
  ...
  const app = {
    ...
    register: vi.fn(),
    addHook: vi.fn(),
    listen: vi.fn(async (opts: { host: string; port: number }) => `http://${opts.host}:${opts.port}`),
    close: vi.fn(async () => {}),
    _routes: routes,
  };
...
describe("Server Bootstrap", () => {
  ...
  describe("/health handler", () => {
    ...
    it("returns correct capability count", async () => {
      const caps = new CapabilityRegistry();
      caps.register({ name: "cap1", domain: "test", description: "d", input: {} as any, handler: async () => ({}) } as any);
  ...
  describe("/ready handler", () => {
    ...
    it("returns 503 when DB is unreachable", async () => {
      const db = {
        execute: vi.fn(async () => { throw new Error("connection refused"); }),
      };
