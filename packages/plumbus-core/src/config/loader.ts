...
function loadDatabaseConfig(env: Record<string, string | undefined>, environment: Environment): DatabaseConfig {
  const defaults = environment === "development"
    ? { host: "localhost", port: 5432, database: "plumbus_dev", user: "postgres", password: "postgres" }
...
function loadQueueConfig(env: Record<string, string | undefined>, environment: Environment): QueueConfig {
  return {
...
function loadAuthConfig(env: Record<string, string | undefined>, environment: Environment): AuthAdapterConfig {
  return {
