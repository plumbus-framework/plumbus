...
describe("OverrideStore", () => {
  ...
  it("includes non-expired overrides", () => {
    ...
    const store = createOverrideStore([
      { rule: "valid", justification: "ok", author: "dev", timestamp: new Date().toISOString(), expiresAt: future },
    ]);
    expect(store.getOverrides()).toHaveLength(1);
  ...
  it("hasOverride checks scope", () => {
    const store = createOverrideStore([
      { rule: "test", justification: "ok", author: "dev", timestamp: new Date().toISOString(), scope: "entity:User" },
    ]);
...
describe("applyOverrides", () => {
  const signals: GovernanceSignal[] = [
    ...
    { severity: GovernanceSeverity.Warning, rule: "rule-b", description: "B", affectedComponent: "y" },
