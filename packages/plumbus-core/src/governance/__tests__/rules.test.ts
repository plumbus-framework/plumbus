...
describe("Architecture Rules", () => {
  it("flags capabilities with excessive effects", () => {
    ...
    const signals = ruleExcessiveEffects.evaluate(
      emptyInventory({
        capabilities: [
          mockCap({ effects: { data, events: [], external: [], ai: false } }),
        ],
      }),
    );
    expect(signals).toHaveLength(1);
...
describe("AI Rules", () => {
  ...
  it("flags AI capabilities without explanation", () => {
    const signals = ruleAIWithoutExplanation.evaluate(
      emptyInventory({
        capabilities: [
          mockCap({ effects: { data: [], events: [], external: [], ai: true } }),
        ],
      }),
    );
    expect(signals).toHaveLength(1);
  ...
  it("flags excessive AI usage", () => {
    ...
    const signals = ruleExcessiveAIUsage.evaluate(
      emptyInventory({ capabilities: caps }),
    );
    expect(signals).toHaveLength(1);
