...
export const ruleCapabilityMissingAccessPolicy: GovernanceRule = {
  ...
  evaluate(inventory) {
    return inventory.capabilities
      .filter((cap) => !cap.access || (!cap.access.roles?.length && !cap.access.scopes?.length && !cap.access.public))
      .map((cap) => ({
        severity: GovernanceSeverity.High,
...
export const ruleOverlyPermissiveRoles: GovernanceRule = {
  ...
  description: "Capabilities should not use overly permissive roles like '*' or empty role arrays with public access",
  evaluate(inventory) {
    return inventory.capabilities
...
export const ruleCrossTenantDataAccess: GovernanceRule = {
  ...
  evaluate(inventory) {
    ...
    return inventory.capabilities
      .filter((cap) => cap.effects.data?.some((d) => nonTenantEntityNames.has(d)) && cap.access?.tenantScoped)
      .map((cap) => ({
        severity: GovernanceSeverity.High,
        ...
        affectedComponent: `capability:${cap.name}`,
        remediation: "Set tenantScoped: true on the accessed entities or remove tenant scope from the capability",
      }));
  },
};
