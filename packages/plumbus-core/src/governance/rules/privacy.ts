...
const sensitiveNamePattern =
  /email|password|ssn|social_security|credit_card|phone|address|dob|date_of_birth|name|salary|medical|health|tax|bank/i;
  ...
  evaluate(inventory) {
    return inventory.entities.flatMap((entity) =>
      Object.entries(entity.fields)
        .filter(
          ([fieldName, fieldDef]) =>
            sensitiveNamePattern.test(fieldName) && !fieldDef.options.classification,
        )
        .map(([fieldName]) => ({
          severity: GovernanceSeverity.Warning,
