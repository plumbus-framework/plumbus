...
describe('generateFlowTrigger', () => {
  const flow: FlowTriggerInput = {
    ...
    startPath: '/api/billing/request-refund',
  };
  ...
  it('generates a flow trigger function', () => {
    ...
    expect(code).toContain('/api/billing/request-refund');
    expect(code).not.toContain('/refund-approval/start');
    expect(code).toContain('method: "POST"');
  });
  ...
  it('omits a discovered flow with no declared HTTP start route', () => {
    expect(generateFlowTrigger({ name: 'onboarding' })).toBe('');
    const module = generateClientModule([], [{ name: 'onboarding' }]);
    expect(module).not.toContain('startOnboarding');
    expect(module).not.toContain('OnboardingFlowInput');
    expect(module).not.toContain('/start');
  });
...
describe('generateClientModule', () => {
  ...
  it('generates a complete client module', () => {
    ...
    const flows: FlowTriggerInput[] = [
      { name: 'refundFlow', domain: 'billing', startPath: '/api/billing/request-refund' },
    ];
