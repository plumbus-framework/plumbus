...
export interface FlowTriggerInput {
  /** Explicit HTTP start route supplied by the host; flow discovery alone exposes no route. */
  startPath?: string;
  name: string;
  domain?: string;
  description?: string;
...
export function generateFlowTrigger(
  ...
  const response = await fetch(${JSON.stringify(`${base}${urlPath}`)}, {
    method: "POST",
...
export function generateClientModule(
  ...
  const routedFlows = flows.filter((flow) => flow.startPath);
  const lines: string[] = [
