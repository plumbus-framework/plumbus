...
- **Don't** generate code that depends on `@plumbus/ui` at runtime — generated code imports from React, a client module, or `@/lib/*` / `@/hooks/*` paths only.
...
- **Don't** edit auto-generated files (`lib/client.ts`, `lib/auth.ts`, `lib/form-hints.ts`, `hooks/hooks.ts`) manually — they are regenerated.
...
```bash
# Scaffold a Next.js app (includes generated modules in lib/ and hooks/)
plumbus ui nextjs frontend

# Regenerate UI modules after changing capabilities
# Auto-detects frontend/ and writes to lib/ and hooks/
plumbus ui generate
```
...
| Client imports | `import { functionName } from "../lib/client"` |
| Auth imports | `import { getStoredToken, ... } from "@/lib/auth"` |
