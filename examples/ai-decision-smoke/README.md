...
both the Laya and TypeSafe adapters against it. **No TypeSafe account is needed.**
...
boundary; `ctx.ai.decide()` is still deferred. No database is required.
...
The server is left running after a successful smoke. Use `stop` when finished.
Model files are cached between starts. Read [the provider guide](../../docs/ai/decision-providers.md)
for the protocol and the remaining core integration work.
